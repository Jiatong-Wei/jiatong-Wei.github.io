/* ============================================================
   魏佳桐 LEO WEI — 单屏面板交互层
   1. 视图切换:标签 → .view 显隐(hash 同步 #v-xxx)
   2. i18n:中英切换(data-i18n 字典注入,localStorage 持久)
   3. 主题:浅/深切换(localStorage 持久,浅色默认)
   零依赖 · vanilla JS
   ============================================================ */
(function () {
  "use strict";
  ...
  var I18N = {
    zh: {
      mark: "魏佳桐 · 机器人学习",
      tab_isaac: "旗舰项目", tab_log: "研究动态", tab_about: "关于",
    ...
    en: {
      mark: "Jiatong Wei · Robot Learning",
      tab_isaac: "Flagship", tab_log: "Log", tab_about: "About",
  ...
  function showView(name) {
    var hit = false;
    views.forEach && views.forEach(function (v) {
      var on = v.id === "v-" + name;
      v.classList.toggle("is-on", on);
      if (on) hit = true;
    });
    if (!hit && views.length) views[0].classList.add("is-on");
    tabs.forEach && tabs.forEach(function (t) {
      t.classList.toggle("is-on", t.getAttribute("data-view") === name);
      t.setAttribute("aria-selected", t.getAttribute("data-view") === name ? "true" : "false");
    });
